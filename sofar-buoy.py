...
try:
    ...
    print('starting from incremental nc file. First burst', dsold['time'][0].values,  'last burst', dsold['time'][-1].values)
...
dsnew = xr.Dataset()
dsnew['time'] = xr.DataArray(pd.DatetimeIndex([x['timestamp'] for x in lines]), dims='time')
...
def add_standard_attrs(ds):
    ...
    ds['significantWaveHeight'].attrs['standard_name'] = 'sea_surface_wave_significant_height'
    ds['significantWaveHeight'].attrs['units'] = 'm'

    ds['peakPeriod'].attrs['standard_name'] = 'sea_surface_wave_period_at_variance_spectral_density_maximum'
    ds['peakPeriod'].attrs['units'] = 's'

    ds['meanPeriod'].attrs['standard_name'] = 'sea_surface_wave_mean_period'
    ds['meanPeriod'].attrs['units'] = 's'

    ds['peakDirection'].attrs['standard_name'] = 'sea_surface_wave_from_direction_at_variance_spectral_density_maximum'
    ds['peakDirection'].attrs['units'] = 'degree'

    ds['peakDirectionalSpread'].attrs['standard_name'] = 'sea_surface_wave_directional_spread_at_variance_spectral_density_maximum'
    ...
    ds['meanDirectionalSpread'].attrs['standard_name'] = 'sea_surface_wave_directional_spread'
...
try:
    if hour == '00':
        shutil.copy(fildir + site + '.nc', fildir + '../waves_bak/' + site + timestr + '.nc')
except:
